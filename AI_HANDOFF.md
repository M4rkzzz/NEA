...
- 自动签到是用户显式开启的独立功能。只在检测到 OOPZ 进程正在运行且当前注册表登录态可读时工作；OOPZ 未运行、未登录或登录态无效时保持等待，不启动、关闭、重启或切换客户端。
...
- 自动轮询每分钟观察一次运行/登录状态，实际网络检查至少间隔 10 分钟；同一账号同一自然日确认成功后不再发请求。手动“立即检查”可跳过间隔，但仍不得重复签到。
...
pnpm run verify:release  # 仅正式发布：构建、格式、82 项稳定测试、Clippy
```
...
1. 同步更新 `package.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 中的 `nea` 根包、`src-tauri/tauri.conf.json` 和 README 版本。
