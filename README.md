...
[![Release](https://img.shields.io/badge/release-v1.3.3-2563eb)](https://github.com/M4rkzzz/NEA/releases/latest)
...
- OOPZ 新增“自动签到”：开启后只在客户端已登录并运行时静默检查，当天尚未签到才自动完成，不弹窗、不切号，也不自动领取里程碑奖励。
- Steam 账密切号改为同时确认目标 `ActiveUser` 与本轮 CM 在线事件，稳定处理连接日志轮转、断线重连和 VDF 写入竞态，失败后界面会刷新真实状态。
- 主进程启动初始化全面后台化；数据恢复、配置迁移和完整托盘构建不再阻塞开屏动画。初始化失败或超时只显示不可操作的加载或错误页。
- 分享导入移除不可靠的无状态 HTTP 预检，继续保留 Cookie 域、SteamID 归属和隔离 WebView 恢复校验；真实混合分享包往返验证通过。
- 自动签到使用当前 Windows 用户已有的 OOPZ 登录态，只连接官方 HTTPS 网关，不保存额外密码或把登录态上传到 NEA 服务。

完整变更与安装包见 [NEA v1.3.3 Release](https://github.com/M4rkzzz/NEA/releases/tag/v1.3.3)。

## 安装与使用

1. 从 [Releases](https://github.com/M4rkzzz/NEA/releases/latest) 下载 `NEA_1.3.3_x64_en-US.msi`。
