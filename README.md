...
[![Release](https://img.shields.io/badge/release-v1.3.1-2563eb)](https://github.com/M4rkzzz/NEA/releases/latest)
...
- 重构系统托盘菜单，以平台和账号为中心直接切号，移除重复动作前缀和技术字段。
- OOPZ 使用账号昵称；Steam 只显示社区名称，不再把登录账号、备注或 SteamID64 当作主名称。
- 完美账号按“名称 段位分数 信誉”显示，并在账号子菜单中提供“仅切换完美”和“同步切换 Steam + 完美”。
- 当前账号置顶标记；账号处理期间自动禁用重复操作，失败时打开 NEA 显示具体原因。
- 同一平台的同名账号仅在必要时追加 ID 后四位，异常或过长昵称会自动整理为安全的单行标签。
- 历史重复网页登录按 SteamID 合并为一个完美托盘账号，缺少公开名称时使用安全的后四位占位。

完整变更与安装包见 [NEA v1.3.1 Release](https://github.com/M4rkzzz/NEA/releases/tag/v1.3.1)。

## 安装与使用

1. 从 [Releases](https://github.com/M4rkzzz/NEA/releases/latest) 下载 `NEA_1.3.1_x64_en-US.msi`。
