...
[![Release](https://img.shields.io/badge/release-v1.2.6-2563eb)](https://github.com/M4rkzzz/NEA/releases/latest)
...
- Steam 客户端切换统一改为保存账密登录，不再把 `loginusers.vdf` 的缓存记录误判为可复用登录态。
- 登录成功需要实际进程与 `ActiveUser` 连续稳定匹配；超时后会尝试恢复并验证原账号。
- 完善 SteamID64 双阶段查重：重复身份仍会补齐缺失的账密或网页登录能力，而不是直接丢弃有效数据。
- 批量网页登录可即时识别密码错误、手机令牌和邮件验证；二维码出现刷新入口时立即刷新。
- 修复并行导入与多处配置更新的丢失更新竞态，补全流程支持暂停、继续和取消。
- Steam 列表名称使用社区名称（PersonaName），登录账号作为基础信息保留。
- WebView2 登录态与可重建缓存分离，大幅压缩 `web-sessions` 占用；数据目录按用途重新分层。
- 重构完美平台账号卡片、暗色主题、窗口布局与 NEA 应用图标。

完整变更与安装包见 [NEA v1.2.6 Release](https://github.com/M4rkzzz/NEA/releases/tag/v1.2.6)。

## 安装与使用

1. 从 [Releases](https://github.com/M4rkzzz/NEA/releases/latest) 下载 `NEA_1.2.6_x64_en-US.msi`。
