...
[![Release](https://img.shields.io/badge/release-v1.3.2-2563eb)](https://github.com/M4rkzzz/NEA/releases/latest)
...
- 完美托盘菜单增加段位分类，按“完美平台 → 段位 → 账号”快速定位目标账号，并继续提供“仅切换完美”和“同步切换 Steam + 完美”。
- 完美菜单顶部新增“仅显示可用号”开关；开启后隐藏人工标记不可用、高危或需要信誉验证的账号，切换筛选时菜单保持在当前操作位置。
- 账号仍以“完美名称 段位分数 信誉”显示；当前账号会同时标记在对应段位和账号上，资料待检测的账号独立归类。
- 启动时立即显示主题匹配的 NEA 品牌动画，主界面准备完成后平滑淡出，减少 WebView 初始化期间的白屏感。
- 深色与浅色模式使用对应品牌图；系统减少动画设置会跳过动效，OOPZ 插件浮层继续保持透明且不会闪现启动画面。
- Windows 本地构建改为速度优先配置，并增加只对当前 Release 程序重新封装 MSI 的快速命令。

完整变更与安装包见 [NEA v1.3.2 Release](https://github.com/M4rkzzz/NEA/releases/tag/v1.3.2)。

## 安装与使用

1. 从 [Releases](https://github.com/M4rkzzz/NEA/releases/latest) 下载 `NEA_1.3.2_x64_en-US.msi`。
